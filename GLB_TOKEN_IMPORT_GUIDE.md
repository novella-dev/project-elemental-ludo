# GLB token import guide for coding LLMs

## Purpose

This guide explains how to reproduce the working `fire.glb` integration in this Unity project. It is written for another coding LLM, including DeepSeek, and focuses on the shortest reliable editor-only workflow.

The required result is:

- Unity imports `Assets/Models/fire.glb` as a usable `GameObject` asset.
- All four red gameplay tokens use that model in Play Mode.
- Blue, yellow, and green tokens continue using the procedural pawn.
...
Unity does not provide native GLB mesh import in this project. Before installing a glTF importer, Unity handled `fire.glb` with `DefaultImporter`. This made the file visible in the Project window but did not produce a `GameObject`, mesh, hierarchy, or materials that a prefab could reference.
...
Run `git status` first. Preserve all unrelated changes. Confirm that `fire.glb` and its `.meta` file exist.
...
This keeps model choice data-driven and reusable. To add water, earth, or air tokens later, assign a different model to the corresponding player-style asset without duplicating token movement code.
...
2. Read the optional model and fitting values from `PlayerStyle`.
...
8. Preserve imported materials and use `MaterialPropertyBlock` for selectable/disabled feedback.
...
### 8. Assign fire.glb to RedPlayerStyle

Preferred method: select `RedPlayerStyle.asset` in Unity and assign `fire.glb` to **Token Model** in the Inspector. Set:

```text
Token Model Euler Angles: (-90, 0, 0)
Token Model Footprint: 0.68
Token Model Height: 0.98
```
...
The resulting serialized reference in this exact revision is:
...
2. Confirm the Editor log imports `fire.glb` with `ScriptedImporter`.
3. Open `Assets/Scenes/LudoBoard3D.unity`.
4. Enter Play Mode.
5. Confirm there are four red tokens.
6. Confirm every red token has a child named `__TokenModel` with at least one renderer.
7. Confirm the procedural renderer on each red `TokenVisual` is disabled.
8. Confirm other colours still show procedural pawns.
9. Roll a 5 and take a red token out of Home.
10. Confirm the fire model moves with the token and no static copy remains in its former Home position.
11. Select a red token to confirm the existing root collider and interaction state still work.
12. Exit Play Mode and run `git diff --check`.
...
An editor validation pass temporarily saved `m_Enabled: 0` overrides for the four red procedural renderers. Those overrides were removed. The renderer must be enabled in the prefab and disabled dynamically only while the custom model exists in Play Mode.
...
Do not change imported materials directly to display selectable or disabled states. That can affect all instances. Use `MaterialPropertyBlock`.
...
- [ ] Confirm `fire.glb` loads as a `GameObject`.
...
- [ ] Preserve imported materials and interaction feedback.
- [ ] Assign the model to red only.
- [ ] Remove the empty manual scene placeholder.
- [ ] Run an editor compilation and Play Mode smoke test.
- [ ] Move a fire token out of Home and verify no static copy remains.
