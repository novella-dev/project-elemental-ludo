...
namespace ElementalLudo.Board
{
    ...
    public sealed class LudoBoardRenderer : MonoBehaviour
    {
        ...
        {
            const float outerEdge = 7.5f;
            const float centerEdge = 1.5f;
            const float lineWidth = 0.025f;

            AddRect(-7.68f, -7.68f, 7.68f, 7.68f, GridColor, 0.08f);
            AddRect(-outerEdge, -outerEdge, outerEdge, outerEdge, BoardWhite, 0.06f);

            // Finishing lanes.
            AddRect(-0.5f, centerEdge, 0.5f, 6.5f, Red, 0.02f);
            AddRect(centerEdge, -0.5f, 6.5f, 0.5f, Blue, 0.02f);
            AddRect(-0.5f, -6.5f, 0.5f, -centerEdge, Yellow, 0.02f);
            AddRect(-6.5f, -0.5f, -centerEdge, 0.5f, Green, 0.02f);

            // Entry squares.
            AddRect(-1.5f, 3.5f, -0.5f, 4.5f, Red, 0.015f);
            AddRect(3.5f, 0.5f, 4.5f, 1.5f, Blue, 0.015f);
            AddRect(0.5f, -4.5f, 1.5f, -3.5f, Yellow, 0.015f);
            AddRect(-4.5f, -1.5f, -3.5f, -0.5f, Green, 0.015f);
